...
from eurocode_calculator.services.materials_adapter import get_steel_fyk
...
def verify_column_buckling(request: ColumnBucklingRequest) -> ColumnBucklingResponse:
    ...
    Utilise eurocodepy pour la limite d'élasticité de l'acier (principal) et un catalogue
    interne de profilés. Le calcul du facteur de réduction χ suit la méthode analytique EC3.
    """
    fyk = get_steel_fyk(request.steel_grade)

    # Propriétés de profilés laminés (catalogue interne)
